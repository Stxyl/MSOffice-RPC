    ...
    if user == stored_user and auth_hash == stored_pwd:
        ...
        class GUI:
            def __init__(self, master):
                ...
                self.Word = Button(master, text="Word", command=Word)
                self.Word.pack()

                self.PowerPoint = Button(master, text="PowerPoint", command=PowerPoint)
                self.PowerPoint.pack()

                self.Excel = Button(master, text="Excel", command=Excel)
                self.Excel.pack()

                self.OneNote = Button(master, text="OneNote", command=OneNote)
                self.OneNote.pack()

                self.Outlook = Button(master, text="Outlook", command=Outlook)
                self.Outlook.pack()

                self.Teams = Button(master, text="Teams", command=Teams)
                self.Teams.pack()
